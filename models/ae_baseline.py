...
class baseline_discriminator(nn.Module):
    def __init__(self, args):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=5, padding=2),  # batch, 64, 108, 89
            nn.LeakyReLU(0.2, True),
            nn.AvgPool2d(kernel_size=2, stride=2)  # batch, 64, 54, 44
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=5, padding=2),  # batch, 64, 54, 44
            nn.LeakyReLU(0.2, True),
            nn.AvgPool2d(kernel_size=5, stride=2),  # batch, 64, 27, 22
            nn.AvgPool2d(kernel_size=5, stride=2),  # batch,64, 11, 8
        )
